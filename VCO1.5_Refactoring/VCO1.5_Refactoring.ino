/*
  ...
  TODO later (as a separate project):
  ...
  6. Replace multiplexing with look-up tables (like using a quater-logsine lookup table) (maybe?)
  7. Make it possible to select different wavetables (as another easter egg mode, probably)
...
// Easter egg mode: OUT2 = OUT1 + 90 degrees in FM and TAH mode
// How to enter easter egg mode: all pots (OSC PITCH, OSC TIMBRE, WAVESHAPE, and probably PITCH MOD and TIMBRE MOD) at max
// And mode is set to TAH (mode is pulled to VCC, connected to the "+" pin)
// See synth.cpp for a define that disables mode check at startup (only knobs are required to be set to max to enter easter egg mode)
